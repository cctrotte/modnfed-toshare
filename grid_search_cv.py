...
if __name__ == "__main__":
    ...
    #ids_ = [ 257,  318,  303, 342,  290, 311, 270, 317, 507]
    ...
    for fold in range(5):
        ...
        assert all(set(s["train"]).isdisjoint(s["valid"]) and set(s["train"]).isdisjoint(s["test"]) and set(s["valid"]).isdisjoint(s["test"]) for s in node_splits_fold.values())
        ...
        for combi in combinations:
            ...
            hidden_layers_enc = param_set["hidden_layers"]
            hidden_layers_dec = param_set["hidden_layers"]

            # hidden_layers_enc = param_set["hidden_layers_enc"]
            # hidden_layers_dec = param_set["hidden_layers_dec"]
            mu = param_set["mu"]
            if model_type == "modn":
            ...
            best_models["fl"]["global"] = {"config": {}, "state_dict": {}, "auprc": 0, "epoch": 0}
            best_models["centralized"]["global"] = {"config": {}, "state_dict": {}, "auprc": 0, "epoch": 0}
            ...
            for round_num in range(federated_rounds):
                ...
                with torch.no_grad():
                    ...
                    current_auprc_fl = np.mean([best_models["fl"][node_id]["auprc"] for node_id in range(1, len(node_data_train)+1)])
                    current_auprc_local = np.mean([best_models["local"][node_id]["auprc"] for node_id in range(1, len(node_data_train)+1)])
                    current_auprc_centralized = np.mean([best_models["centralized"][node_id]["auprc"] for node_id in range(1, len(node_data_train)+1)])
                    if current_auprc_fl > best_auprc_fl or current_auprc_local > best_auprc_local or current_auprc_centralized > best_auprc_centralized:
                        if current_auprc_fl > best_auprc_fl:
                            best_models["fl"]["global"]["state_dict"] = global_model.state_dict()
                            best_models["fl"]["global"]["config"] = global_config
                            best_models["fl"]["global"]["auprc"] = current_auprc_fl
                            best_models["fl"]["global"]["f1"] = current_f1_fl
                            best_models["fl"]["global"]["epoch"] = round_num
                            best_auprc_fl =current_auprc_fl
                        if current_auprc_local > best_auprc_local:
                            best_auprc_local = current_auprc_local
                        if current_auprc_centralized > best_auprc_centralized:
                            best_auprc_centralized = current_auprc_centralized
                            best_models["centralized"]["global"]["state_dict"] = centralized_model.state_dict()
                            best_models["centralized"]["global"]["config"] = centralized_config
                            best_models["centralized"]["global"]["auprc"] = current_auprc_centralized
                            best_models["centralized"]["global"]["f1"] = current_f1_centralized
                            best_models["centralized"]["global"]["epoch"] = round_num
            ...
            summary = run.summary  # summary is a Summary object
            row = {
                "fold":   fold,
                **param_set,           # your hyperparams for this run
                # pull out whatever keys you logged as summary metrics:
                "fl_auprc_best":        summary.get("fl_auprc_best"),
                "local_auprc_best":     summary.get("local_auprc_best"),
                "best_fl_epochs":    summary.get("best_fl_epochs"),
                "centralized_auprc_best": summary.get("centralized_auprc_best"),
                "fl_f1_best":           summary.get("fl_f1_best"),
                "local_f1_best":        summary.get("local_f1_best"),
                "centralized_f1_best":  summary.get("centralized_f1_best"),
                "FL_better_than_local_f1": summary.get("FL_better_than_local_f1"),
                "FL_better_than_local_auprc": summary.get("FL_better_than_local_auprc"),
            ...
            filename = "__".join(name_parts) + "__" + str(fold)
    ...
    # save as CSV
    csv_path = os.path.join(base_dir, "cv_metrics.csv")
